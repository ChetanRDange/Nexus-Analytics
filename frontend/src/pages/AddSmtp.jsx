...
const AddSmtp = () => {
  ...
  const validate = () => {
    ...
    if (!formData?.fromName?.trim()) {
      errors.fname = from_name_required_error;
      toast.error(from_name_required_error);
      return errors;
    }
    if (!formData?.fromEmail?.trim()) {
      errors.fmail = from_email_required_error;
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    const validateSmtpDetails = validate();
    if (validateSmtpDetails) {
      setErrors(validateSmtpDetails);
      return;
    }
    if (Object.keys(validateSmtpDetails).length === 0) {
      const smtpDetails = {
        ...formData,
        port: Number(formData?.port),
        secure: statusDropdown.name,
      };
      const res = await callApi("/private/smtp", "POST", smtpDetails);
  ...
  return (
    <>
      ...
      <div className="h-full py-8 p-4 sm:p-5 overflow-x-hidden">
        ...
        <div className="w-full justify-center items-center mt-8 pb-12 gap-y-4 gap-2 lg:items-start md:items-end xl:items-end ">
        ...
        <div className="w-full flex justify-end items-center gap-4 pt-8 pb-20 mb-20">
          <Link
