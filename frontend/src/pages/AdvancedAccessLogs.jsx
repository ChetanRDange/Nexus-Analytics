...
const AdvanceAccessLogs = () => {
  const { fullScreen } = useContext(GlobalContext);
  const [triggerRefetch, setTriggerRefetch] = useState(false);

  const tableMetaData = {
    tableName: "advanceAccessLogs",
    model: "Advance Access Logs",
    endpoint: "/private/logs/advanceAccess",
    viewPath: "advance-access-logs/view",
    showDeleteAll: true,
    deleteField: "type",
  };
      ...
      <div className=" w-full">
        {!fullScreen && (
          <div className="w-full flex justify-between pb-5 border-b border-primary ">
            <div className="">
              <h4 className="text-3xl text-dark">API Logs</h4>
            </div>
          </div>
        )}
...
export default AdvanceAccessLogs;
