...
const AddDocument = () => {
  ...
  if (id) {
    const { data: res } = useFetch(`/private/documentation/${id}`);
  ...
  const addContentField = () => {
    const newIndex = documentDetails.content.length;
    setDocumentDetails({
      ...documentDetails,
      content: [
    ...
    setCollapsedSections((prevSections) => [...Array(newIndex).keys()]);
  };
  ...
  const validate = () => {
    ...
    documentDetails.content.forEach((contentItem, index) => {
      if (!contentItem.linkTitle?.trim()) {
        newErrors.content[index].linkTitle = "Title is required.";
        hasErrors = true;
      }

      if (!contentItem.link?.trim()) {
        newErrors.content[index].link = "Link is required.";
        hasErrors = true;
      } else if (!urlRegex.test(contentItem.link?.trim())) {
        newErrors.content[index].link = "Invalid Link format";
        hasErrors = true;
      }
      ...
      if (!contentItem.description?.trim()) {
        ...
        newErrors.content[index].description =
  ...
  const hanldeAddDocument = async (e) => {
    e.preventDefault();
    ...
    if (!isValid) {
      Object.values(errors).forEach((error) => {
        if (typeof error === "string" && error) {
          ...
          error.forEach((contentError) => {
            Object.values(contentError).forEach((fieldError) => {
              if (fieldError) toast.error(fieldError);
            });
          });
        }
      });
  ...
  return (
    <>
      <div className="py-8 p-4 sm:p-5 overflow-x-hidden mb-20">
        <div className="w-full pb-8 gap-y-4 gap-2 flex flex-col justify-between items-center   ">
        ...
        <div className="w-full justify-center items-center mt-8 pb-8 gap-y-4 gap-2 lg:items-start md:items-end xl:items-end ">
          <div className="w-full sm:w-[85%] md:w-[80%] lg:w-[90%] xl:w-[74%] 2xl:w-[60%] flex flex-col gap-y-2 md:flex-row justify-evenly">
            ...
            <div className="w-full">
              ...
              <div className="w-full mt-5">
                <label className="text-primary ">Contents</label>
                            ...
                            <div className="flex items-center gap-2 pointer-events-auto">
                              ...
                              {documentDetails.content.length > 1 && (
                                <button
                                  ...
                                  onClick={() =>
                                    removeContentField(contentIndex)
                                  }
                                  id="button-129"
                                >
                                  ✕
                      ...
                      <>
                        <div className="w-full mt-2">
                          <InputComponent
                            ...
                            errorMessage={
                              errors.content[contentIndex]?.linkTitle
                            }
                            onChange={(e) =>
                              handleContentChange(
                                contentIndex,
                ...
                <div className="w-full mt-4">
                  <button
                    type="button"
                    className="px-4 py-2 text-white font-medium bg-primary hover:bg-primarydark primarydark:text-primarydark rounded-xl whitespace-nowrap"
              ...
              <div className="w-full mt-5">
                <ToggleComponent
                  ...
                  tooltipMessage="Publish the document"
                />
              </div>
            </div>
          </div>
